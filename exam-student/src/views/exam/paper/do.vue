  ...
  created () {
    ...
    if (id && parseInt(id) !== 0) {
      ...
      examPaperApi.select(id).then(re => {
        ...
        let mycache = localStorage.getItem(this.form.id)
        if (mycache !== null) {
          this.answer = JSON.parse(mycache)
          this.remainTime = this.remainTime - this.answer.doTime
        }
        _this.timeReduce()
      })
    }
  ...
  watch: {
    answer: {
      ...
      handler: function (newAnswer, oldAnswer) {
        localStorage.setItem(this.form.id, JSON.stringify(newAnswer))
      }
    }
  },
  mounted () {
